...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if not self.flash:
            print("Flash Attention not supported. Using normal attention.")
    ...
    def forward(self, x, attn_mask):
        ...
        if self.flash:
            ...
            out = F.scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0, is_causal=True)
...
class GPT(nn.Module):
    ...
    def generate_raw(self, idx: Tensor, max_tokens_generate: int, temperature, top_k, top_p, ):
        # idx (B, T)
    ...
    def generate(self, tokenizer, out_fn, progress, idx: Tensor, max_tokens_generate: int, temperature: int = 1, top_k: int = 0, top_p: float = 1.0):
        # idx (B, T)
